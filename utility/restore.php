...
<title>Discuz! Công cụ khôi phục dữ liệu</title>
<style type="text/css">
...
<div class="bodydiv">
	<h1>
		Discuz! Công cụ khôi phục dữ liệu
		<span> &nbsp; Nếu bạn có bất kỳ câu hỏi nào trong quá trình khôi phục, vui lòng truy cập trang web hỗ trợ kỹ thuật <a href="http://www.discuz.net" target="_blank">http://www.discuz.net</a></span>
	</h1>
	<div class="main">
EOT;
}
...
function show_msg($message, $url_forward = '', $type = 'message', $success = 0) {
	...
	if($type == 'message') {
		...
		echo "$message ...";
		echo "<br /><br /><br /><a href=\"$url_forward\">Trình duyệt sẽ tự động chuyển đến trang mà không cần can thiệp thủ công. Trừ khi trình duyệt của bạn không tự động chuyển hướng trong một thời gian dài, vui lòng nhấp vào đây</a>";
		echo "<script>setTimeout(\"redirect('$url_forward');\", 1250);</script>";
	} elseif($type == 'confirm') {
		echo "$message";
		echo "<br /><br /><br /><button id=\"confirmbtn\" onclick=\"redirect('$url_forward')\">Xác nhận</button><button id=\"cancelbtn\" onclick=\"redirect('{$siteurl}restore.php')\">Hủy bỏ</button>";
	}
...
function lang($lang_key, $force = true, $replace = array()) {
	$lang = array('filename' => 'Tên tập tin',
				'director' => 'Danh mục',
				'version' => 'Phiên bản',
				...
				'db_volume' => 'Số',
				'import' => 'Nhập khẩu',
				'different_dbcharset_tablepre' => 'Người ta phát hiện thấy dữ liệu sao lưu đã nhập khác với {diff} của tệp cấu hình. Bạn có muốn tiếp tục chạy chương trình này không?',
				'db_import_tips' => 'Chức năng này sẽ ghi đè hoàn toàn dữ liệu gốc khi khôi phục dữ liệu sao lưu. Vui lòng đảm bảo rằng diễn đàn đã được đóng trước khi khôi phục và diễn đàn có thể được mở lại sau khi quá trình khôi phục hoàn tất.<br />Bạn có thể xem thông tin chi tiết về các tệp sao lưu của trang web thông qua chức năng quản lý sao lưu dữ liệu, xóa các bản sao lưu đã hết hạn và nhập các bản sao lưu bắt buộc.<br /><span class="red">Toàn bộ quá trình khôi phục dữ liệu sẽ được hoàn tất sang một trang mới, sau khi bạn khôi phục dữ liệu thành công thì hãy xóa kịp thời file restore.php.</span><br />',
				'db_export_discuz' => 'Discuz! Dữ liệu (không bao gồm UCenter)',
				'db_export_discuz_uc' => 'Dữ liệu Discuz! Và UCenter',
				'db_multivol' => 'Dung lượng lớn',
				'db_import_unzip' => 'Giải nén',
				'db_export_zip' => 'Sao lưu nén',
				'db_zip' => 'ZIP',
				'db_shell' => 'Shell',
				'unknown' => 'Không xác định',
				'backup_file_unexist' => 'Tệp sao lưu không tồn tại',
				'connect_error' => 'Không kết nối được với cơ sở dữ liệu, vui lòng kiểm tra xem các tệp cấu hình cơ sở dữ liệu config/config_global.php và config/config_ucenter.php có tồn tại hay không và cấu hình có đúng không',
				'dbcharsetdiff' => ' Bộ ký tự cơ sở dữ liệu($_config[\'db\'][\'1\'][\'dbcharset\'])',
				'tableprediff' => ' Bảng Prefix($_config[\'db\'][\'1\'][\'tablepre\'])',
				'database_import_multivol_succeed' => 'Dữ liệu khối lượng phụ được nhập thành công vào cơ sở dữ liệu trang web<br />Vui lòng cập nhật bộ nhớ cache trong nền<br /><span class="red">Vui lòng xóa tệp restore.php càng sớm càng tốt để tránh ảnh hưởng đến dữ liệu</span>',
				'database_import_file_illegal' => 'Tệp dữ liệu không tồn tại: máy chủ có thể không cho phép tải tệp lên hoặc kích thước tệp vượt quá giới hạn',
				'database_import_multivol_prompt' => 'Tập dữ liệu con của tập đầu tiên đã được nhập thành công vào cơ sở dữ liệu. Bạn có cần tự động nhập các tập con khác của bản sao lưu này không?',
				'database_import_succeed' => 'Dữ liệu đã được nhập thành công vào cơ sở dữ liệu trang web<br />Vui lòng cập nhật bộ đệm ẩn trong nền<br /><span class="red">Vui lòng xóa tệp restore.php càng sớm càng tốt để tránh ảnh hưởng đến dữ liệu</span>',
				'database_import_format_illegal' => 'Tệp dữ liệu không ở định dạng Discuz! Và không thể nhập được',
				'database_import_unzip' => '{info}<br />Sau khi tệp sao lưu được giải nén, bạn có cần nhập bản sao lưu tự động không? Tệp đã giải nén sẽ bị xóa sau khi nhập',
				'database_import_multivol_unzip' => '{info}<br />Sau khi tệp sao lưu được giải nén, bạn có cần tự động giải nén các tệp ổ đĩa con khác không?',
				'database_import_multivol_unzip_redirect' => 'Tệp dữ liệu #{multivol} được giải nén thành công, chương trình sẽ tự động tiếp tục',
				'database_import_confirm' => 'Việc nhập dữ liệu không phù hợp với phiên bản Discuz! Hiện tại rất có thể gây ra lỗi không thể giải quyết được. Bạn có chắc chắn tiếp tục không?',
				'database_import_confirm_sql' => 'Bạn có chắc chắn nhập bản sao lưu này không?',
				'database_import_confirm_zip' => 'Bạn có chắc chắn giải nén bản sao lưu này không?',
				'database_import_multivol_confirm' => 'Sau khi tất cả các tập tin khối lượng phụ được giải nén, bạn có cần tự động nhập bản sao lưu không? Các tệp được giải nén sau khi nhập sẽ bị xóa',
				'database_import_multivol_redirect' => 'Đã nhập thành công tệp dữ liệu #{volume}, chương trình sẽ tự động tiếp tục',
				'error_quit_msg' => 'Các vấn đề trên phải được giải quyết để tiếp tục khôi phục dữ liệu',
				'restored_error' => 'Chức năng khôi phục dữ liệu đã bị khóa và đã được khôi phục. Nếu bạn chắc chắn khôi phục được dữ liệu, vui lòng xóa ./data/restore.lock trên máy chủ',
			);
